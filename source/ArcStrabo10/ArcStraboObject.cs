    ...
    class ArcStraboObject
    {
        ...
        public void textLayerExtract(string sourceImageDir, string resultImageDir)
        {
            try
            {
                ...
                fillImageList(resultImageDir + "\\" + ArcStrabo10Extension.TextPositiveLabelLayerJSONFileName, imgfnp_list, srcImage);
                fillImageList(resultImageDir + "\\" + ArcStrabo10Extension.TextNegtiveLabelLayerJSONFileName, imgfnn_list, srcImage);
                ...
                {
                    int unique_number = 0;
                    while (File.Exists(ArcStrabo10Extension.Text_Result_Path + "\\" + ArcStrabo10Extension.TextLayerPNGFileName))
                    {
                        unique_number += 1;
                        if (File.Exists(ArcStrabo10Extension.Text_Result_Path + "\\" + ArcStrabo10Extension.TextLayerPNGFileName))
                        {
                            ArcStrabo10Extension.TextLayerPNGFileName = "TextLayer(" + unique_number.ToString() + ").png";
                        }
                    }
                }
                catch (Exception e)
                {
                    Log.WriteLine(ArcStrabo10Extension.ErrorMsgNameTextLayer);
                    Log.WriteLine(e.Message);
                    Log.WriteLine(e.ToString());
                }

                result_list[0].Save(resultImageDir + "\\" + ArcStrabo10Extension.TextLayerPNGFileName);
            }
            catch (Exception e)
            {
        ...
        public IWorkspace CreateShapefileWorkspace(string dir)
        {
            try
            {
                ...
                //if (System.IO.Directory.Exists(dir + "\\"+ArcStrabo10Extension.Result_Shapefile_Folder_Name))
                //    System.IO.Directory.Delete(dir + "\\" + ArcStrabo10Extension.Result_Shapefile_Folder_Name, true);

                // create new folder for shapefile if one already exists
                try
                {
                    int unique_number = 0;
                    while (System.IO.Directory.Exists(dir + "\\" + ArcStrabo10Extension.Result_Shapefile_Folder_Name))
                    {
                        unique_number += 1;
                        if (System.IO.Directory.Exists(dir + "\\" + ArcStrabo10Extension.Result_Shapefile_Folder_Name))
                        {
                            ArcStrabo10Extension.Result_Shapefile_Folder_Name = "ocr_shapefile(" + unique_number.ToString() + ")";
                            ArcStrabo10Extension.TextLayerOCRShapefile = "OCRLayer(" + unique_number.ToString() + ")";
                        }
                    }
                }
                catch (Exception e)
                {
                    Log.WriteLine(ArcStrabo10Extension.ErrorMsgNameOCRLayer);
                    Log.WriteLine(e.Message);
                    Log.WriteLine(e.ToString());
                }

                IWorkspaceName workspaceName = workspaceFactory.Create(dir,
                 ArcStrabo10Extension.Result_Shapefile_Folder_Name, null, 0);
