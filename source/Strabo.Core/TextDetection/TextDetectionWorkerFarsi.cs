...
{
   public class TextDetectionWorkerFarsi
    {
        ...
        public string Apply(string input_dir, string output_dir, string fn, double size_ratio, bool preprocessing)
        {
          ...
          //  Bitmap srcimg = new Bitmap(input_dir + fn);
            ...
            List<Bitmap> img_list = imgslicer.Apply(1, 1, 100, srcimg);
            //srcimg.Dispose();
            //srcimg= null;
            ...
            for (int s = 0; s < img_list.Count; s++)
            {
                ...
                ConditionalDilationFarsi cda = new ConditionalDilationFarsi();
          ...
          //      List<TextString> string_list = detectTS.Apply(img_list[s], dilatedimg);
                DetectTextStringsFarsi detectTSFarsi = new DetectTextStringsFarsi();
                List<TextString> string_list = detectTSFarsi.Apply(img_list[s], dilatedimg);
                //time = Log.GetDurationInSeconds();
            ...
            Log.WriteBitmap2FolderExactFileName(output_dir, srcimg, "debug.png");
            g.Dispose();
            //time = Log.GetDurationInSeconds();
